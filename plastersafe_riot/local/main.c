...
static char TOPIC_TO_PUBLISH[] = "simulated_data";
...
static int read_data(int argc, char **argv)
{
    (void)argc;
    (void)argv;
    get_data('0');
...
static int start(int argc, char **argv)
{
    ...
    if(reachable){
        ...
        while(1){
            char *msg = get_data_from_sensors();
            strcat(msg, id);
...
static const shell_command_t shell_commands[] = {
    ...
    {"read", "read data from sensors", read_data},
...
int main(void)
{
    puts("PlasterSafe MQTT-SN Station.\n");
